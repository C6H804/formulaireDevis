...
INSERT INTO
    sondage_types (name)
VALUES (
        "publicité grand mail st paul lès dax"
    );

INSERT INTO
    sondage_types (name)
VALUES (
        "publicité papier (catalogue, prospectus, flyer)"
    );

INSERT INTO
    sondage_types (name)
VALUES (
        "publicité extérieure (panneau publicitaire)"
    );

INSERT INTO
    sondage_types (name)
VALUES (
        "sur la route, en passant devant l'agence à bégaar"
    );
...
SELECT projects_types.name, COUNT(CASE WHEN stats_devis.date >= '2026-02-01' AND stats_devis.date <= '2026-02-28' THEN stats_projects.id_project_Type END) AS nombre_devis FROM projects_types 
...
GROUP BY projects_types.name ORDER BY projects_types.name ASC;